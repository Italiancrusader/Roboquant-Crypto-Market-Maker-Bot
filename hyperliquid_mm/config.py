...
def load_config(path: str, require_keys: bool = True) -> BotConfig:
    ...
    cfg = BotConfig(
        ...
        max_inventory_usd=float(risk.get("max_inventory_usd", 200.0)),
        session_loss_limit_usd=float(risk.get("session_loss_limit_usd", 25.0)),
        flatten_on_exit=bool(risk.get("flatten_on_exit", False)),
